          ...
          <p align="justify" style="margin: 5px;font-size: 17px">
            <font face="cambria">
              <!-- Bapak/Ibu dari <?php $instansiID = $this->m_pengguna->getPenggunaByID($penggunaID)->id_instansi;
                                  ...
                                  } ?> yang terhormat, saat ini kami sedang melakukan <i><b>Tracer Study</b></i> (penelusuran alumni) <b>Program Studi <?php echo $this->m_master->getProdiByID($prodiID)->nama_prodi ?></b> UNIVERSITAS. Adapun tujuan dari kegiatan ini adalah untuk mendapatkan basis data yang diperlukan dalam penyusunan Evaluasi Diri dalam rangka Akreditasi Program Studi. Berkaitan dengan hal tersebut,  kami mohon Bapak/Ibu dapat mengisi kuesioner ini, data yang Bapak/Ibu isi dijamin kerahasiaannya. Untuk kerjasama dan bantuannya, kami mengucapkan banyak terima kasih.  -->
            </font>
                      ...
                      foreach ($pertanyaan as $p) {
                        ...
                        ?>
                          <div class="form-group">
                            ...
                            <input type="text" name="isian_<?php echo $p->id ?>" <?php if ($p->required == 'yes') {
                                                                                    echo "required";
                        ...
                        <?php } elseif ($p->jenis == 'isian' && $p->textarea == 'ya') { ?>
                          <div class="form-group">
                            <label class="form-control-label"><b><?php echo $p->pertanyaan ?>:</b></label>
                            <textarea class="form-control" <?php if ($p->required == 'yes') {
                                                              echo "required";
                                                            } ?> name="isian_<?php echo $p->id ?>" rows="5"></textarea>
                          </div>
                        <?php } elseif ($p->jenis == 'pilihan') { ?>
                          ...
                          foreach ($pilihan as $k) { ?>
                            <div class="i-checks">
                              <input id="radioCustom1" type="radio" name="pilihan_<?php echo $p->id ?>" <?php if ($p->required == 'yes') {
                                                                                                          echo "required";
